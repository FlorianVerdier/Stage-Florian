...
$$\mathbb{P}\left(\underset{0\leq s \leq t}{\sup} |\bar{L}_N(s) - x(s)| \geq \delta \right) < \eta.$$

\begin{align}
\underset{0\leq s \leq t}{\sup} |\bar{L}_N(s) - x(s)| < \delta & \Rightarrow \forall s \in [0,t]\; |\bar{L}_N(s) - x(s)| \geq \delta\\
& \Rightarrow \forall s \in [0,t] \;\bar{L}_N(s) < x(s) + \delta \leq C + \delta <C_{\max}\\
& \Rightarrow \bar{L}_N(s) = \hat{L}_N(s)
\end{align}
car avant que l'on atteigne C, et donc qu'une réservation soit refusée, les deux processus sont égaux
D'où $$\underset{0\leq s \leq t}{\sup} |\bar{L}_N(s) - x(s)| < \delta  \Rightarrow \underset{0\leq s \leq t}{\sup} |\hat{L}_N(s) - x(s)| < \delta $$


De ceci, il vient que $$\mathbb{P}\left(\underset{0\leq s \leq t}{\sup} |\hat{L}_N(s) - x(s)| \geq \delta \right) < \eta.$$
\end{proof}

\subsection{Cas sur-critique ($C<C_{\max)}$)}
Plaçons nous dans le cas $C<C_{\max}$
\begin{theorem}[Limite fluide dans le cas sur-critique]
Dans le cas sur-critique, la limite fluide du système est $$x(t) = \min(\lambda\E{\1{p<a}\min((t-p)^+,\sigma,r)},C)$$ 
\end{theorem}
\begin{proof}
