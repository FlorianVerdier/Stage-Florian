...
Notons $M_N(t) = \int_{(\R^+)^3} \1{B_t}(u,x,y) \left(\mathcal{N}_N(du,dx,dy)-\lambda N du \nu(dx,dy)\right)$ , de sorte que $C_N(t) = M_N(t) + \lambda N \E{C(t)} $

\begin{lemme}
$(M_N(t))$ est une martingale de processus croissant associé $\lambda N \E{C(t)}$
\end{lemme}

Par la suite, pour simplifier l'écriture, on notera $x(s) = \lambda \E{C(s)}$

$\underset{0\leq s \leq t}{\sup} |\bar{C}_N(t)-x(s)| = \frac{1}{N}\underset{0\leq s \leq t}{\sup} |M_N(s)| $
