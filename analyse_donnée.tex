...
Cette démonstration suit une démarche similaire à  effectuée dans l'article [] présentant un calcul similaire pour ce qui s'apparenterait à un système sans temps de pré-réservation.
...
\end{align*}
Comme $\mathbbm{1} = \1{0\leq u+x \leq t} - \1{0\leq u+x+y \leq t}$, on peut séparer l'intégrale en deux:


\begin{align*}
C_N(t) &= \int_{(\R^+)^3} \1{0\leq u+x \leq t}B_t(u,x,y) \mathcal{N}_N(du,dx,dy)\\ &\,\,\,-
\int_{(\R^+)^3} \1{0\leq u+x+y \leq t}B_t(u,x,y) \mathcal{N}_N(du,dx,dy)\\
&= \int_{(\R^+)^3} \1{0\leq u \leq t}B_t(u,x,y) \mathcal{N}_{N,\Phi_1}(du,dx,dy)\\ &\,\,\,-
\int_{(\R^+)^3} \1{0\leq u \leq t}B_t(u,x,y) \mathcal{N}_{N,\Phi_2}(du,dx,dy)\\
\end{align*}

où $\Phi_1$ est défini sur $\R_+^3$ par $\Phi_1(u,x,y) = (u+x,x,y)$  et $\mathcal{N}_{N,\Phi_1}$  est un processus de poisson d'intensité l'image par $\Phi_1$ de l'intensité de $\mathcal{N}_N$, soit $\lambda N \1{u\geq x}du\nu(dx,dy)$

Notons $M_{N,\Phi_1}(t) = \int_{[0,t]\times (\R^+)^2}  \left(\mathcal{N}_N(du,dx,dy)-\1{u\geq x}\lambda N du \nu(dx,dy)\right)$.\\
Pour la filtration associée à $\mathcal{N}_N$, c'est une martingale de de processus croissant associé\\
...
$$\frac{C_N(t)}{N} = \frac{M_{N,\Phi_1}(t)}{N} - \frac{M_{N,\Phi_2}(t)}{N} + \lambda  \E{(t-p)^+-(t-p-\sigma)^+}$$
